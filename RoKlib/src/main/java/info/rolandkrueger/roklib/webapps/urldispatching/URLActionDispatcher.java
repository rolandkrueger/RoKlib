...
/**
 * <p>
 * The central dispatcher which provides the main entry point for the URL action handling framework.
 * The action dispatcher manages one internal root URL action handler which dispatches to its
 * sub-handlers. When a visited URL has to be interpreted, this URL is passed to methdo
 * {@link #handleURIAction(URL, String)} or {@link #handleURIAction(URL, String, ParameterMode)},
 * respectively. There, the URL is split into a token list to be recursively interpreted by the
 * registered action handlers. For example, if the following URL is to be interpreted
 * 
 * <pre>
 * http://www.example.com/myapp/user/home/messages
 * </pre>
 * 
 * with the web application installed under context <code>http://www.example.com/myapp/</code> the
 * URI fragment to be interpreted is <code>/user/home/messages</code>. This is split into three
 * individual tokens <code>user</code>, <code>home</code>, and <code>messages</code> in that order.
 * To interpret these tokens, the root action handler passes them to the sub-handler which has been
 * registered as handler for the first token <code>user</code>. If no such handler has been
 * registered, the dispatcher will do nothing more or return the default action command that has
 * been registered with {@link #set404FileNotFoundCommand(AbstractURLActionCommand)}. It thus
 * indicates, that the URL could not successfully be interpreted.
 * </p>
 * <p>
 * Note that this class is not thread-safe, i.e. it must not be used to handle access to several
 * URLs in parallel. You should use one action dispatcher per HTTP session.
 * </p>
 * 
...
public class URLActionDispatcher implements Serializable
{
  ...
  /**
   * <p>
   * If <code>true</code> is passed, the dispatcher is instructed to ignore a possible exclamation
   * mark at the beginning of the relative path when handling the URI action. That is, e. g., a
   * relative path <code>!user/login</code> will be handled as if the path <code>user/login</code>
   * was given for interpretation. This is useful if your application is a rich internet application
   * that uses URI fragments for making pages bookmarkable. In the example above, the full URL might
   * look like <code>http://www.example.com/#!user/login</code>.
   * </p>
   * <p>
   * Using an exclamation mark after the hash is a scheme proposed by Google to make an AJAX
   * application whose HTML-code is generated by JavaScript crawlable for a web crawler. See <a
   ...
   * </p>
   * <p>
   * By ignoring the exclamation mark, you can have URLs using this particular scheme without having
   * to explicitly deal with the exclamation mark.
   * </p>
   * 
   * @param ignoreExclamationMark
   *          <code>true</code> if an exclamation mark at the beginning of the relative path to be
   *          handled should be ignored
   */
  ...
  /**
   * Returns the root dispatching handler that is the entry point of the URL interpretation chain.
   * This is a special action handler as the URL token it is responsible for (its
   * <em>action name</em>) is the empty String. Thus, if a visited URL is to be interpreted by this
   * action dispatcher, this URL is first passed to that root dispatching handler. All URL action
   * handlers that are responsible for the first directory level of a URL have to be added to this
   * root handler as sub-handlers. To do that, you can also use the delegate method
  ...
  /**
   * Sets the action command to be executed each time when no responsible action handler could be
   * found for some particular relative URI. If set to <code>null</code> no particular action is
   * performed when an unknown relative URI is handled.
   * 
  ...
  public void handleParameters (Map<String, String[]> parameters)
  {
    ...
    for (String key : parameters.keySet ())
    {
      ...
      if (!params.isEmpty ()) mCurrentParameters.put (key, params);
    }
  }
  ...
  /**
   * Passes the given relative URI to the URL action handler chain and interprets all parameters
   * with the {@link ParameterMode} defined with {@link #setParameterMode(ParameterMode)}.
   * 
   ...
   *          <code>http://www.example.com/myapp</code> this is the context passed in through this
   *          parameter. This value may be <code>null</code>.
   * @param relativeUri
   *          relative URI to be interpreted by the URL action handling framework. This may be an
   *          URI such as <code>/admin/configuration/settings/language/de</code>
   * @param parameterMode
   *          {@link ParameterMode} to be used for interpreting possible parameter values contained
   *          in the given relative URI
   * @return an object containing all necessary data for a file download. This return value is
   *         optional and may be <code>null</code>. If a {@link DownloadInfo} object is returned,
   *         the contained {@link InputStream} should be written to the {@link OutputStream} of the
   *         HTTP response.
   */
  public DownloadInfo handleURIAction (URL context, String relativeUri, ParameterMode parameterMode)
  {
    ...
    {
      action.execute ();
      if (mListener != null) mListener.handleURLActionCommand (action);
  ...
  /**
   * Returns the relative URI that is currently being handled by this dispatcher. This URI is set
   * each time {@link #handleURIAction(URL, String, ParameterMode)} is called.
   */
  ...
  /**
   * Adds a new sub-handler to the root action handler of this dispatcher. For example, if this
   * method is called three times with action handlers for the fragments <code>admin</code>,
   * <code>main</code>, and <code>login</code> on a web application running in context
   * <code>http://www.example.com/myapp</code> this dispatcher will be able to interpret the
   * following URLS:
   * 
   * <pre>
