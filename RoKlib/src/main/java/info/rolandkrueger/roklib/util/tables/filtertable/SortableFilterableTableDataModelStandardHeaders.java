    ...
    SortableFilterableTableDataModel<T, DefaultTableDataColumnHeader>
{
  ...
  public SortableFilterableTableDataModelStandardHeaders (int columnCount, SearchMode searchCapability)
  {
    super (columnCount, searchCapability);
  }

  public SortableFilterableTableDataModelStandardHeaders (TableModel tableModel, SearchMode searchCapability)
  {
  ...
  public void setHeaderNames (String... names)
  {
    if (names.length > getColumnCount ())
      throw new IllegalArgumentException (String.format ("Too many column names: %d names > %d columns", names.length,
          getColumnCount ()));
