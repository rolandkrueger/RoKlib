...
public interface ITernarySearchTreeQuery
{
   ...
   * 
   * @param string
   *          The String's <code>toString()</code> method is used to generate a string representation of that String in
   *          order to find a set of partially matching String differing only by a number of <code>distance</code>
   *          characters.
   * @param distance
   *          The number of maximum characters that may be different from the characters of <code>string</code>.
   * @param lengthTolerance
   *          specifies by how many characters the length of the resulting set's strings may differ from the length of
   *          the search key.
   */
