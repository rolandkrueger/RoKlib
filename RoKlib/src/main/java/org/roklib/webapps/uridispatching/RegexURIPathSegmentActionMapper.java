 ...
 * interpreting a visited URI by the {@link URIActionDispatcher}. Note that the array of matched token fragments does
 * not contain {@link Matcher}'s first capturing group holding the entire pattern.
 * </p>
 * <h1>Generating Parameterized Action URIs</h1>
 * <p>
 * When you are generating parameterized action URIs with the {@link #getParameterizedActionURI(boolean)} methods, you
 * have to provide a value for the URI token used to represent this {@link RegexURIPathSegmentActionMapper}. This is done with
 * {@link #setURIToken(String)}. The token set with this method must be able to be successfully matched against the
 * regular expression of this handler. Otherwise, an exception is thrown. If you generate a parameterized action URI
 ...
 * every {@link RegexURIPathSegmentActionMapper} that is found on the path from the action handler tree's root to some action
...
public class RegexURIPathSegmentActionMapper extends DispatchingURIPathSegmentActionMapper {
    ...
    /**
     * The pattern object of this {@link RegexURIPathSegmentActionMapper}. It is compiled in the constructor and each time the case
     * sensitivity is changed.
     */
    private Pattern pattern;

    /**
     * Creates a new {@link RegexURIPathSegmentActionMapper} with the provided regular expression. This regex will be applied to the
     ...
     *
     * @return the set of captured token fragments or <code>null</code> if this {@link RegexURIPathSegmentActionMapper} is not
    ...
    /**
     * Checks if this {@link RegexURIPathSegmentActionMapper} is responsible for handling the given URI token. It does so by
    ...
    /**
     * Retrieves the matched values from the capturing groups of this {@link RegexURIPathSegmentActionMapper}'s regular expression.
     */
    ...
    /**
     * Since the action name of the {@link RegexURIPathSegmentActionMapper} is a regular expression, it should not be converted to
     ...
     * the pattern for this action handler. Otherwise, this {@link RegexURIPathSegmentActionMapper} would not be able to interpret
     ...
     * URI token for every {@link RegexURIPathSegmentActionMapper} that can be found on the path from this action handler back to
